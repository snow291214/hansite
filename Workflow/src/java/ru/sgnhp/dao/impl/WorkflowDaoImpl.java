...
public class WorkflowDaoImpl extends SimpleJdbcDaoSupport implements IWorkflowDao{
    ...
    public List<Workflow> getWorkflowByUserUid(Long userUid) {
        List<Workflow> workflows = getSimpleJdbcTemplate().query(SELECT + "  WHERE  workflows.UserUid = ? order by AssignDate, State", new WorkflowMapper(taskManagerService), userUid);
        return workflows;
    }
