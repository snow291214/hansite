...
public class UserDaoImpl extends SimpleJdbcDaoSupport implements IUserDao {
    ...
    public WorkflowUser getByLogin(String login) {
        List<WorkflowUser> users = getSimpleJdbcTemplate().query("Select * From users Where Login=?", new UserMapper(), login);
        if (users.size() > 0) {
    ...
    public List<WorkflowUser> getAllUsers() {
        List<WorkflowUser> users = getSimpleJdbcTemplate().query(SELECT + " order by LastName", new UserMapper());
        return users;
    }

    public List<WorkflowUser> getNormalisedUsers() {
        List<WorkflowUser> users = getSimpleJdbcTemplate().query(SELECT + " where LastName <> '' order by LastName", new UserMapper());
        return users;
    }
